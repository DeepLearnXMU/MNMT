...
class TranslationBuilder(object):
    ...
    def __init__(self, data, fields, n_best=1, replace_unk=False,
                 has_tgt=False, image_feat_type=None):
        self.data = data
        self.fields = fields
        self.n_best = n_best
    ...
    def from_batch(self, translation_batch):
        ...
        if self.image_feat_type is not None:
            preds, pred_score, attn, attn_img, attn2, gold_score, indices = list(zip(
                *sorted(zip(translation_batch["predictions"],
                            translation_batch["scores"],
                            translation_batch["attention"],
                            translation_batch["attention_img"],
        ...
        for b in range(batch_size):
            ...
            if self.image_feat_type is not None:
                translation = Translation(src[:, b] if src is not None else None,
                                        ...
                                        gold_score[b],attn_img[b], attn2[b])
            else:
                translation = Translation(src[:, b] if src is not None else None,
                                        src_raw, pred_sents,
                                        attn[b], pred_score[b], gold_sent,
...
class Translation(object):
    ...
    def __init__(self, src, src_raw, pred_sents,
                 attn, pred_scores, tgt_sent, gold_score,attn_img=None, attn2=None):
        self.src = src
