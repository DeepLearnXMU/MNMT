...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        # print('x_size',x.size())
        # print('mean_size:',torch.mean(x, 1).size())
        # torch.mean(x, 1).view(x.size(0),1).expand(x.size())
        # torch.sqrt(torch.var(x, 1).expand_as(x))
        x = (x - torch.mean(x, 1).view(x.size(0),1).expand_as(x)) / torch.sqrt(torch.var(x, 1).view(x.size(0),1).expand_as(x) + self.epsilon)
        if self.learnable:
